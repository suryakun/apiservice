...
$(document).ready(function () {
	// Unread and Comment detail
	$('#comment_tab a').click(function(){
		var target = $(this).attr('href');
		
		$('#comment_tab a').removeClass('cur');
		$(this).addClass('cur');
	...
	//Protfolio Detail
	$(".gallery-portfolio").colorbox({rel:'group1'});
	
	//Editable form
	$('.edit-form').click(function(){
		var obj = $(this).parent().next();
		obj.find("input,select,textarea").removeAttr('disabled');
		obj.find(".form-actions").removeClass('hidden');
	});
	...
	$('#post_diary, #post_portfolio, #post_activity, #post_info').click(function(){
		$('#add_diary').show();
	});

	//Form
	var formAll = $('.add-status > .tab-content > .tab-pane');

	//Button
	var new_story = $('.create-new li a');

	new_story.click(function(){
		var formCreate = $(this).attr('href');

		$('#add_diary').show();
		formAll.removeClass('active');
		$(formCreate).addClass('active');
	});
